...
import { setErrorReq, setSuccessReq } from '../../redux/actions/RequestHandler';
...
function ThemeWrapper(props) {
  ...
  const error = useSelector(state => state?.getIn(['requestHandler'])?.error);
  const success = useSelector(state => state?.getIn(['requestHandler'])?.success);
  ...
  useEffect(() => {
    ...
    if (error === true) {
      ...
      dispatch(setErrorReq(false));
    }
  }, [error]);
  useEffect(() => {
    if (success === true) {
      setOpenSuccessBar(true);
      dispatch(setSuccessReq(false));
    }
  }, [success]);
